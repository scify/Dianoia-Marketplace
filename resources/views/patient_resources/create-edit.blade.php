...
@section('content')
    ...
    <form id="md-form" enctype="multipart/form-data" role="form" method="POST"
          action="{{ $viewModel->isEditMode() ? route('resources.update',$viewModel->resource->id) : route('patient_resources.store') }}">
        <input type="hidden" id="type_id" value='{{$viewModel->type_id}}'/>
    @if($viewModel->isEditMode())
        @method('PUT')
    @endif

    <!--form class="md-form" action="{{route('patient_resources.store')}}" method="POST" enctype="multipart/form-data"-->
        {{ csrf_field() }}
        <div class="container rounded py-4" style="border:1px solid grey">
            ...
            <div class="d-flex justify-content-end">
                ...
                <a class="btn btn-outline-primary" href="{{route('patient_resources.index')}}">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="md-modal-form" enctype="multipart/form-data" role="form" method="POST"
                          action="{{route('patient_resources.store')}}">
                        {{ csrf_field() }}
