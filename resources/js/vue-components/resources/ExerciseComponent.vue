  ...
  <div v-if="resource.id">
      ...
      <div class="exercise-box" v-bind:class="[isCarerExercise() ? 'carer-template' :'patient-template']">
        ...
        <div class="exercise-title-row d-flex justify-content-between align-items-center">
          ...
          <div style="padding-right:15px">
            <button v-if="isProfilePage() || loggedInUserIsAdmin()" type="submit" class="btn btn--edit"
                    @click="showDeleteModal()"><i class="fa fa-trash" title="Delete"
                                                  style="font-size:25px;color:red"></i></button>
            <button v-if="isProfilePage() || loggedInUserIsAdmin()" type="submit" class="btn btn--edit"
                    @click="showEditModal"><i class="far fa-edit" title="Edit"></i></button>
            <button v-if="loggedInUserIsAdmin()  && isApproved()" type="submit" class="btn btn--edit"
                    @click="showExerciseRejectionModal"><i class="fas fa-thumbs-down" title="Reject"
                                                           style="font-size:30px;color:red"></i></button>
            <button v-else-if="loggedInUserIsAdmin() && isRejected()" type="submit" class="btn btn--edit"
                    @click="approveExercise"><i class="fas fa-thumbs-up" title="Approve"
                                                style="font-size:30px;color:var(--color-green)"></i></button>
            <button v-else-if="loggedInUserIsAdmin() && isPending()" type="submit" class="btn btn--edit"
                    @click="approveExercise"><i class="fas fa-thumbs-up" title="Approve"
                                                style="font-size:30px;color:var(--color-green)"></i></button>
            <button v-if="loggedInUserIsAdmin() && isPending()" type="submit" class="btn btn--edit"
                    @click="showExerciseRejectionModal"><i class="fas fa-thumbs-down" title="Reject"
                                                           style="font-size:30px;color:red"></i></button>
            <button type="submit" class="btn btn--report" @click="showExerciseReportModal"><i
                class="fas fa-exclamation-triangle hover-red " title="Report"
                style="font-size:20px;color:rgba(255,0,0,0.1);padding-right:15px;"></i></button>

            <a :href="'/storage/'+resource.pdf_path" class="btn btn--secondary"
               :class="{ disabled: !resource.pdf_path }" target="_blank" rel="noopener noreferrer">{{
              trans('messages.see-exercise') }}</a>
          </div>

        </div>

        <div
            style="padding-left:15px;padding-top:15px; border-top:1px solid red; box-shadow: 1px 1px red; align-items:center;"
            v-if="resource.reportData">
          <i v-for="user in this.users" :key="user.id">
            <div class="reported-by" v-if="user.id===resource.reportData.reporting_user_id"><a style="color:red"><u>Reported
              by {{user.name}}</u></a></div>
          </i>
          <i class="fas fa-envelope fa-3x" @click="showResponseModal" title="Respond to reporting user"
             style="color:var(--color-green)"> <span @click="showResponseModal"
                                                     style="color:var(--color-green); font-size: large; padding-top:10px; padding-bottom: 10px"> Respond </span></i>
          <p>Reason for Report: <span style="color:red; font-size: medium; font-style:italic">{{resource.reportData.reason}}</span>
          </p>
          <p>Reporter Comments: <span style="color:red; font-size:medium; font-style:italic">{{resource.reportData.comment}}</span>
          </p>
        </div>
        <hr>
        ...
        <div class="exercise-rating p-4 d-flex justify-content-between align-items-center">
          <div class="rating">
            <span v-for="index in maxRating" :key="'rating_' + index" class="fa-star"
                  v-bind:class="{ fas: resourceHasRating(index), far: !resourceHasRating(index) }"></span>
            <p class="rating-btn" v-if="loggedInUserIsDifferentFromContentUser()" @click="showRateModal">{{ trans('messages.give-rating')
              }}</p>
          </div>

          <i v-for="user in this.users" :key="user.id">
            <div class="created-by" v-if="user.id===resource.creator_user_id">{{ trans('messages.created-by')
              }}{{user.name}}
            </div>
          </i>
          ...
          <i v-for="difficulty in this.difficulties" :key="'difficulty_' + difficulty.id">
            <div class="level" v-if="difficulty.id===resource.difficulty_id">{{trans('messages.'+difficulty.name)}}
            </div>
          </i>
    ...
    <modal-component
      ...
      <template v-slot:footer>
        ...
        <div>
          <button type="button" class="btn btn--secondary me-5" @click="closeEditModal">{{trans('messages.cancel')}}
          </button>
          <button v-on:click="getEditExerciseRoute()" type="button" class="btn btn--primary">{{trans('messages.yes')}}
          </button>
        </div>
      </template>
